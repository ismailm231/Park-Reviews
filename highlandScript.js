async function fetchAndDisplayReviews() {
    ...
    try {
        const response = await fetch('http://localhost:3000/parks/highland/reviews');
...
async function updateReview(userName, rating, description, timeOfVisit, keyWords) {
    try {
        ...
        await fetch(`http://localhost:3000/parks/highland/reviews/${userName}`, {
            method: 'PUT',
            headers: {
...
async function deleteReview(userName) {
    try {
        ...
        const response = await fetch(`http://localhost:3000/parks/highland/reviews/${userName}`);
...
window.onload = function () {
    ...
    document.getElementById('review-form').addEventListener('submit', async function (event) {
        ...
        try {
            ...
            await fetch('http://localhost:3000/parks/highland/reviews', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    document.getElementById('update-review-button').addEventListener('click', async function () {
        ...
        if (updateUserNameInput) {
            try {
                ...
                const response = await fetch(`http://localhost:3000/parks/highland/reviews/${updateUserNameInput}`);
    ...
    document.getElementById('delete-review-button').addEventListener('click', async function () {
        ...
        if (deleteUserNameInput) {
            try {
                ...
                const response = await fetch(`http://localhost:3000/parks/highland/reviews/${deleteUserNameInput}`);
                const review = await response.json();

                if (review) {
                    
                    await fetch(`http://localhost:3000/parks/highland/reviews/${deleteUserNameInput}`, {
                        method: 'DELETE',
                    });
